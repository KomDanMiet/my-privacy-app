...
async function refreshToken(tokenJson: any) {
  ...
  try {
    ...
    return {
      ...
      // set absolute expiry_date if provided
      expiry_date:
        j.expires_in ? Date.now() + Number(j.expires_in) * 1000 : tokenJson.expiry_date,
    };
  } catch (e) {
    console.warn("[scan] refresh exception:", e);
    return tokenJson;
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    // 2) refresh if expired/missing access_token
    const isExpired =
      tokenJson?.expiry_date && Number(tokenJson.expiry_date) < Date.now() - 30_000;
    ...
    // 3) list recent messages & fetch From headers
    const q = `newer_than:${Math.ceil(days / 30)}m`; // coarse filter: past N months
    ...
    // 4) upsert into discovered_senders
    if (domainCounts.size) {
      ...
      // If your table has a unique index on (email, domain)
      // this upsert will replace count with the new count snapshot.
    ...
    // 5) mark scanned_at to stop auto loops
    await sb
      .from("gmail_tokens")
      .update({ scanned_at: nowIso, updated_at: nowIso })
      .eq("email", email);
